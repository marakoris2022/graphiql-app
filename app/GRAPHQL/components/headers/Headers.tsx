...
const Headers: FC<HeadersProps> = ({}) => {
  ...
  const [headers, setHeaders] = useState(() => {
    ...
    // Используем search.keys() для получения всех ключей
    const keys = [...search.keys()];

    // Итерируем по ключам и используем search.get() для получения значений
    for (const key of keys) {
      const value = search.get(key); // Получаем значение по ключу
      if (value) {
        arr.push({ key, value });
      }
    }
